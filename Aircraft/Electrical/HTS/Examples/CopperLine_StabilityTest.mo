...
model CopperLine_StabilityTest
  ...
  Stekly.Stekly_TransmissionLosses stekly_TransmissionLosses(
    l=1,
    n=15.2,
    I_c0=1000,
    A_cu=1e-4,
...
equation
  ...
  connect(fixedTemperature1.port, thermalConductor1.port_a)
    annotation (Line(points={{-40,20},{-30,20}},
                                               color={191,0,0}));
  connect(thermalConductor1.port_b, stekly_TransmissionLosses.port_a)
    annotation (Line(points={{-10,20},{0,20},{0,4}}, color={191,0,0}));
  connect(stekly_TransmissionLosses.pin_n, ground1.p)
    annotation (Line(points={{9,0},{30,0},{30,-40},{0,-40}}, color={0,0,255}));
  connect(stekly_TransmissionLosses.pin_p, rampVoltage1.p)
    annotation (Line(points={{-9,0},{-36,0},{-36,-8}}, color={0,0,255}));
