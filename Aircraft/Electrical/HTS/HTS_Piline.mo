...
model HTS_Piline "HTS line using Stekly equations"
  ...
  Modelica.Electrical.Analog.Basic.Ground ground1
    annotation (Placement(transformation(extent={{22,-32},{30,-24}})));
  Modelica.Blocks.Sources.RealExpression realExpression4(y=C_pi)
    annotation (Placement(transformation(extent={{4,-18},{14,-10}})));
  Modelica.Electrical.Analog.Basic.VariableInductor
                                            inductor2(
    Lmin=0,
    IC=IC,
...
equation
  ...
  R_pi = 1e-3;//l*E_0*DymolaModels.Functions.Math.divNoZero((pin_p.i/I_c)^n,pin_p.i);
  ...
  connect(capacitor1.p, resistor3.n)
    annotation (Line(points={{26,-8},{26,0},{6,0}}, color={0,0,255}));
  connect(capacitor1.n, ground1.p)
    annotation (Line(points={{26,-20},{26,-24}},          color={0,0,255}));
  connect(capacitor1.C, realExpression4.y)
    annotation (Line(points={{18.8,-14},{14.5,-14}}, color={0,0,127}));
  connect(resistor3.p, inductor1.p) annotation (Line(points={{-6,0},{-18,0},{-18,
          16},{-6,16}}, color={0,0,255}));
  connect(inductor2.n, inductor1.p)
    annotation (Line(points={{-32,16},{-6,16}}, color={0,0,255}));
  connect(resistor3.p, resistor2.n)
    annotation (Line(points={{-6,0},{-32,0}}, color={0,0,255}));
  connect(resistor.n, resistor2.p)
    annotation (Line(points={{-58,0},{-44,0}}, color={0,0,255}));
  connect(inductor2.L, realExpression2.y) annotation (Line(points={{-38,23.2},{-38,
          34},{-43.5,34}}, color={0,0,127}));
  connect(resistor2.R, realExpression.y) annotation (Line(points={{-38,-7.2},{-38,
          -10},{-44,-10},{-44,-30},{-51.4,-30}}, color={0,0,127}));
  connect(inductor2.p, resistor2.p) annotation (Line(points={{-44,16},{-48,16},{
          -48,0},{-44,0}}, color={0,0,255}));
  connect(resistor4.n, resistor1.p)
    annotation (Line(points={{46,0},{60,0}}, color={0,0,255}));
  connect(resistor4.p, resistor3.n)
    annotation (Line(points={{34,0},{6,0}}, color={0,0,255}));
  connect(resistor4.R, realExpression.y) annotation (Line(points={{40,-7.2},{40,
          -30},{-51.4,-30}}, color={0,0,127}));
  connect(inductor3.p, inductor1.n)
    annotation (Line(points={{34,16},{6,16}}, color={0,0,255}));
  connect(inductor3.n, resistor1.p)
    annotation (Line(points={{46,16},{52,16},{52,0},{60,0}}, color={0,0,255}));
  connect(inductor3.L, realExpression2.y)
    annotation (Line(points={{40,23.2},{40,34},{-43.5,34}}, color={0,0,127}));
