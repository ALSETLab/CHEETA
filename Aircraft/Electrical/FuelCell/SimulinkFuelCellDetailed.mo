...
model SimulinkFuelCellDetailed "Basic hydrogen fuel cell from MATLAB"
  ...
  Real Eoc "Open circuit voltage";
  Real i0 "Exchange current";
  Real A "Tafel slope";
  Real ifc "Current from circuit";
  Real GAIN "input to transfer function";
  ...
  Modelica.Electrical.Analog.Interfaces.PositivePin pin_p1
        ...
        transformation(extent={{54,-50},{74,-30}}),
            ...
            {80,-30}})));
  Modelica.Blocks.Sources.RealExpression realExpression(y=GAIN)
  ...
  Modelica.Blocks.Sources.RealExpression realExpression1(y=Eoc)
...
equation
  Eoc = K_c * E_n;
  i0 = exp(-dG/R_mol*T)*(z*F*k*(PH2 + PO2)*dV)/ (R_mol *h);
  A = (R_mol*T)/(z*alpha*F);
  ifc = resistor.n.i;
  GAIN = A*log(ifc/i0);
  ...
  connect(pin_p1, signalVoltage.n)
    annotation (Line(points={{64,-40},{34,-40},{34,-32}}, color={0,0,255}));
  connect(realExpression.y, transferFunction.u)
    annotation (Line(points={{-29,-46},{-22,-46}}, color={0,0,127}));
  connect(signalVoltage.v, feedback.y)
    annotation (Line(points={{22,-22},{15,-22}}, color={0,0,127}));
  connect(transferFunction.y, feedback.u2)
    annotation (Line(points={{1,-46},{6,-46},{6,-30}}, color={0,0,127}));
  connect(feedback.u1, realExpression1.y)
    annotation (Line(points={{-2,-22},{-29,-22}}, color={0,0,127}));
