...
equation
  ...
  connect(thermalConductor.port_a, stekly3.port_a)
    annotation (Line(points={{8,76},{24,76},{24,22}}, color={191,0,0}));
  connect(stekly2.port_a, stekly3.port_a)
    annotation (Line(points={{22,36},{22,22},{24,22}},
                                               color={191,0,0}));
  connect(converterVoltageInput.p2, stekly2.pin_p)
    annotation (Line(points={{2,30},{8,30},{8,32},{13,32}},
                                              color={0,0,255}));
  connect(converterVoltageInput.v, voltage_ce.y)
    annotation (Line(points={{0,12},{0,-4},{-13,-4}},   color={0,0,127}));
  connect(electricDrive.flange, fan2.flange_a1)
    annotation (Line(points={{94,24},{113,24}}, color={0,0,0}));
  connect(electricDrive.pin_p, circuitBreaker1.n1)
    annotation (Line(points={{74,30},{57.8,30}}, color={0,0,255}));
  connect(circuitBreaker1.p1, stekly2.pin_n) annotation (Line(points={{38,30},{
          34,30},{34,32},{31,32}}, color={0,0,255}));
  connect(circuitBreaker2.n1, electricDrive.pin_n)
    annotation (Line(points={{57.8,18},{74,18}}, color={0,0,255}));
  connect(stekly3.pin_n, circuitBreaker2.p1)
  ...
  connect(stekly3.pin_p, converterVoltageInput1.n2) annotation (Line(points={{
          15,18},{14,18},{14,-40},{2,-40}}, color={0,0,255}));
