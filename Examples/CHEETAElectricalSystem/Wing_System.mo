...
model Wing_System "CHEETA Electrical System for one wing"
  ...
  Modelica.Blocks.Sources.Constant const5(k=733.038285)
    annotation (Placement(transformation(extent={{-46,-48},{-32,-34}})));
  ElectrifiedPowertrains.ElectricDrives.ESM.ElectroMechanical.SpeedFOC
    speedFOC_ESM3(
    redeclare ElectrifiedPowertrains.ElectricMachines.ESM.Controllers.Speed
      controller(redeclare
    ...
    redeclare
      ...
      machine(redeclare
        Aircraft.Electrical.Machines.Records.CHEETA_Records.CHEETA_2_5MW_ESM
        data)) annotation (Placement(transformation(extent={{-16,-70},{4,-50}})));
...
equation
  ...
  connect(inertia3.flange_a, speedFOC_ESM3.flange)
    annotation (Line(points={{20,-60},{4,-60}}, color={0,0,0}));
  connect(inertia3.flange_b, constantTorque1.flange)
    annotation (Line(points={{40,-60},{54,-60}}, color={0,0,0}));
  connect(const5.y, speedFOC_ESM3.desiredSpeed)
    annotation (Line(points={{-31.3,-41},{-6,-41},{-6,-48}}, color={0,0,127}));
  connect(speedFOC_ESM3.pin_p, speedFOC_ESM.pin_p) annotation (Line(points={{
          -16,-54},{-24,-54},{-24,70},{-16,70}}, color={0,0,255}));
  connect(speedFOC_ESM3.pin_n, ground.p)
    annotation (Line(points={{-16,-66},{-60,-66},{-60,-76}}, color={0,0,255}));
